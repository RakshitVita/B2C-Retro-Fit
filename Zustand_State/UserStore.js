...
const useUserStore = create((set) => ({
  ...
  fetchUserStatus: async () => {
    ...
    try {
      const response = await axiosInstance.get("/user/status"); // replace with your endpoint(GET API)
  ...
  validateFileUpload: async (file) => {
    ...
    try {
      ...
      "/api/validate-file-lines", //VALIDATE FILE API (POST)
        formData
      );
  ...
  convertFile: async (file, language) => {
    ...
    try {
      //API endpoint for file conversion is "/api/convert-file"(POST )
